...
interface ImportClassModalProps {
  grades: Grade[];
  onClose: () => void;
  onImport: (gradeId: number, className: string, students: {name: string, gender: Gender}[]) => void;
}

const ImportClassModal: React.FC<ImportClassModalProps> = ({ grades, onClose, onImport }) => {
  const { t } = useLocale();
  const [selectedGradeId, setSelectedGradeId] = useState<number>(grades[0]?.id || 0);
  ...
  const handleImport = () => {
    if (!className.trim() || !selectedGradeId) return;
    ...
    onImport(selectedGradeId, className.trim(), students);
  };
      ...
      >
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
            <div className="flex justify-between items-center">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white">{t('importClassTitle')}</h3>
        ...
        <div className="p-6 flex-grow flex flex-col space-y-4">
            <div>
                ...
                >
                    {grades.map(g => <option key={g.id} value={g.id}>{g.name} {t('class')}</option>)}
                </select>
            </div>

            <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t('className')}</label>
                <input
                    type="text"
                    value={className}
            ...
            <div className="flex-grow flex flex-col">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t('pasteStudentList')}</label>
                <textarea
                    value={text}
                    onChange={(e) => setText(e.target.value)}
                    className="w-full flex-grow bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg p-3 focus:ring-2 focus:ring-yellow-400 focus:border-yellow-400 transition font-mono text-sm h-40"
        ...
        <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
            ...
            <button
                ...
                disabled={!text.trim() || !className.trim()}
...
export default React.memo(ImportClassModal);
